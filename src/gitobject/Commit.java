...
public class Commit extends GitObject {
    ...
    /**
     * Construct a commit directly from a file. We no longer use it in our proj with
     * stage.
     * 
     * @param
    ...
    /**
     * Restoring the commit object according to its key(commitId), and reading its
     * content
     * 
     * @param commitId
     * @throws IOException
    ...
    /**
     * Use the linked list to get the history of the current commit history, limit
    ...
    /**
     * Use the linked list to get the history of the current commit history， show
     * history after a commit.
     * 
     * @throws IOException
     */
    public void logCommitHistory(String topCommitId) throws IOException {
        System.out.println(getValue());
        if (getKey().equals(topCommitId)) {
            return;
        }
        String parentId = getParent();
        while (parentId != null) {
            ...
            System.out.println(commit.getValue());
            parentId = commit.getParent();
            if (commit.getKey().equals(topCommitId)) {
                break;
            }
        }
    }
